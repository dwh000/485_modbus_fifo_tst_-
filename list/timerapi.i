#line 1 "User\\src\\TimerAPI.c"
#line 1 ".\\User\\inc\\TimerAPI.h"
...
#line 2 "User\\src\\TimerAPI.c"



static u32bit volatile	uTimer0DelayCnt = 0;
u32bit volatile			bIsSetTime0Delay = 0;
TIMER_RELAY_T volatile Timer0Relay;


static TIMER_DELAY_T	Timer0Delay[16];
...
{
	if(bIsSetTime0Delay == 0){
		DrvTIMER_Init();									
		DrvTIMER_Open(E_TMR0,10000,E_PERIODIC_MODE);		
		DrvTIMER_SetTimerEvent(E_TMR0,10,Timer0CallBack,0);	
		bIsSetTime0Delay = 1;
	}
}
u32bit Timer0SetDelay(DELAY_TNO DelayTxx,u32bit nTick)		
{
	if(uTimer0DelayCnt >= 16)
		return (((1) ? 0xFFFF0000 : 0x00000000) | ((((MODULE_ID_DRVTIMER) & 0xFF) | ((1) ? 0x100 : 0x00)) << 7) | ((7) & 0x7F));
	uTimer0DelayCnt++;
	if (Timer0Delay[DelayTxx].active  == 0){
		Timer0Delay[DelayTxx].active       = 1;
...
{
	Timer0Relay.TimerRelayArray[DelayTxx] 	= 0;
	if(Timer0Delay[DelayTxx].active == 1){
		Timer0Delay[DelayTxx].active = 0;
		uTimer0DelayCnt--;
	}
}
